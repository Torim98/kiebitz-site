...
scripts/contract-annex.mjs         self-contained contract annex from /de/terms/
...
legal/                             self-contained contract annex for the billing email
...
`/legal/kiebitz-vertragsbedingungen.html`. That file is generated by
`scripts/contract-annex.mjs` from the freshly built `/de/terms/` page, so the
binding German version exists only once: change the terms template, run the
build, and the attachment follows.

Unlike every other page it is a single self-contained HTML5 document — inline
`<style>`, no script, no font, no image, no relative link — because it is
opened years later from a mail archive with no server behind it. It carries no
navigation, no language selector and no marketing, it is `noindex` so it does
not compete with `/de/terms/`, and it prints on paper with the web addresses
spelled out. `npm run check` enforces all of it: `lang="de"`, the inline style,
the absence of scripts and external stylesheets, absolute references only,
sections 1 to 12, the withdrawal instruction and the model withdrawal form.
